...
import { Box,Image,Text,Grid,GridItem,Center, Heading,Select, HStack} from "@chakra-ui/react";
...
const Products = () => {
  ...
  const locationRoute = currentPath.split('/')[1]; // Extract the first part of the path
  const route = locationRoute.charAt(0).toUpperCase() + locationRoute.slice(1);
  ...
  const filteredArr = selectedFilter ? newArr.filter(item => item.category==selectedFilter) : [...newArr];
  // console.log(newArr);
  ...
  return (
    <>
    <Center>
      <Text mt={{base : 5,md : 10}} fontSize='sm'>HM.com /<Text color='red' as='b'> {route} </Text ></Text>
    </Center>
    <Box display='flex' gap={{base: '20px', md: '40px', lg: '50px'}} p='30px'>
      <Box w={{base:'0%',md:'35%',lg:'35%'}}><Sidebar/></Box>
      <Box>
        {/* <Box>
          <Image src={modelImg} alt='image'></Image>
        </Box> */}
        <Heading size='md' textAlign={{ base: 'center', md : 'left' }}>Product Related To {route}</Heading>
        <HStack spacing={5}>
          <Select placeholder='SORT BY' w='150px' bg='white' border='none' mt='10px' onChange={sortChange} value={selectedOption}>
          <option value='low'>Lowest price</option>
          <option value='high'>Highest price</option>
        </Select>
        <Select placeholder='PRODUCT TYPE' w='180px' bg='white' border='none' onChange={filterChange} value={selectedFilter}>
        ...
        <Grid templateColumns={{ base: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)', lg: 'repeat(4,1fr)' }} gap={4} mt='10px'>
          { filteredArr.length > 0 ?
              filteredArr.map((product,i) => (
                <Link to={`/details/${product.id}`}  key={i} >
                <GridItem>
                  <Image src={product.image} alt='image'  _hover={{ transform: "scale(1.1)", transition: "400ms" }}/>
                  <Text fontSize='sm' mt='5'>{product.title}</Text>
                  <Text fontSize='sm' mt='1'>Rs.{product.price}</Text>
                </GridItem>
                </Link>
              ))
              :<Center><Heading> No products found for this category! </Heading></Center>
          } 
          </Grid>
      </Box>
    </Box>
